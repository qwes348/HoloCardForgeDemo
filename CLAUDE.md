...
### 검증 하네스가 기능과 싸울 수 있다
`PackOpeningRecorder` 의 `advanceEvery` 가 결과 화면에서도 계속 `Go(1)` 을 불러서
**확대해 놓은 걸 하네스가 스스로 풀어 버렸다.** 기능이 고장 난 줄 알고 한참 봤다.
자동 진행은 해당 상태에서 멈추게 하고, 입력이 필요한 상호작용은 `Capture` 의
`onFrame` 콜백으로 원하는 프레임에 끼워 넣을 것 (레코더는 포인터·키보드를
흉내 낼 수 없다 — 실제 장치를 본다).
...
### BoxCollider 는 메시가 붙기 **전에** 만들면 1x1x1 로 굳는다
`AddComponent<BoxCollider>()` 시점에 `MeshFilter` 가 비어 있으면 자동 크기가
기본값(1x1x1)으로 잡히고, 그 뒤 메시를 넣어도 **갱신되지 않는다.** 카드는 실제로
0.64 x 0.88 x 0.012 인데 판정은 정육면체라 클릭이 엉뚱한 데서 먹는다.
→ 절차 생성 메시에는 콜라이더 크기를 **손으로 적을 것.**

### 초점을 옮길 때는 이전 것의 자세를 전부 되돌린다
확대·선택처럼 "한 놈만 특별한" 상태에서 대상을 옮길 때, 이전 대상을 위치만
되돌리고 스케일·회전을 빼먹기 쉽다. 방금 확대했던 카드가 큰 채로 격자에 앉아
혼자 튀었다. 되돌릴 속성은 바꾼 속성과 **같은 목록**이어야 한다.
