...
마지막 장에서 한 번 더 넘기면 뭉치가 격자로 펼쳐진다(`OpenGallery`).
**순환시키지 않는 게 핵심이다** — 무한히 돌면 "다 봤다" 는 순간이 없어서 개봉이
안 끝난다. `‹` 로 뭉치로 돌아가고, `R` 로 다시 뽑는다.

- 카드가 **한 장씩** `galleryStagger` 간격으로 날아가 자리를 잡는다. 한꺼번에
  펼치면 그냥 배치가 바뀐 것으로 보이고, 차례로 놓이면 세는 맛이 생긴다
- 뭉치 순서가 등급 오름차순이라 **제일 좋은 카드가 마지막에 앉는다**
- 카드마다 ◇/★ 표식이 도착 직후 붙는다. 캐러셀용 표식 한 벌은 가운데 한 장 기준으로
  자리를 잡으므로 격자에는 못 쓴다 — `RarityDisplay.ShowGallery` 가 카드 수만큼
  따로 깔고(필요할 때 만들어 재사용), 카드가 앉는 순서에 맞춰 띄운다
- 펼친 카드는 **전부 살려 둔다.** 각도가 안 변하면 포일이 죽어서 결과 화면이
  인쇄물 사진처럼 보인다

#### 카드 확대

격자에서 카드를 클릭하면 앞으로 끌어내 확대한다(`ZoomTo`). 나머지는 어둡게 깔고
살짝 뒤로 민다 — 그냥 확대만 하면 뒤의 카드들이 같은 밝기로 남아서 어느 쪽을
보라는 건지 안 읽힌다.

- 확대한 카드만 컨트롤러를 살리고 `trackOutsideBounds` 도 켠다. 확대해서 보는
  목적이 결국 포일이라 각도가 안 변하면 크게 볼 이유가 없고, 카드 위에서만
  반응하면 조금만 벗어나도 뚝 끊긴다
- 확대 중 화살표·좌우 키는 **옆 카드로 넘어간다.** 확대를 푸는 게 아니라 —
  크게 보다가 옆 것도 크게 보고 싶은 게 자연스럽다. 푸는 건 클릭이나 `ESC`
- 격자 표식은 접는다. 표식은 격자 자리에 붙박이라, 카드 하나가 앞으로 나오면
  그 카드의 표식만 빈자리에 홀로 남아 어느 카드 것인지 안 읽힌다
- 확대 대상을 옆으로 옮길 때는 이전 카드의 **자세를 전부** 되돌린다. 위치만
  되돌리면 방금 확대했던 카드가 큰 채로 격자에 앉아 혼자 튄다
...
투영된 기울기 `length(n.xy)` 로 재야 같은 범위가 0~0.5 로 펼쳐진다.

#### 색을 만드는 법 — 칠하지 말고 **어리게** 한다

은박의 정체는 "무지개색 빛" 이 아니라 "밝은 금속에 색이 **어린** 것" 이다.
색상값을 그대로 칠하면(회색↔색상 `lerp`, 또는 `hue` 를 곱하기) 기울일수록 순수한
무지개가 되어 금속이 아니라 *빛나는 테두리*로 읽힌다. 실물을 확대해 보면 바탕은
끝까지 밝은 은색이고 색은 그 위에 얹힌 편차일 뿐이다.

```hlsl
float  lum  = dot(hue, float3(0.33, 0.5, 0.17));
float3 look = _BorderSilver + (hue - lum) * chroma;   // 은색 바탕 + 색차만
```

결(streak)은 두 겹이다. 한 겹이면 사인파 줄무늬로 보이고, `pow` 로 날을 세우지
않으면 그냥 그라디언트다. 그리고 **색상은 둘레 위치와 결 좌표 양쪽에서 나온다** —
둘레 위치만 쓰면 한 변 안에서 색이 거의 안 변해서 변마다 단색인 띠가 된다.
실물은 이웃한 결끼리 초록·청록·자홍으로 갈라진다.

#### 화면에 얹는 법 — 여기서 두 번 틀렸다

색을 아무리 잘 만들어도 합성에서 씻긴다. 두 가지를 조심할 것.

1. **채널마다 따로 포화시키지 말 것.** `1 - exp(-x)` 를 채널별로 걸면 밝은 채널이
   먼저 눌려서 `(1.8, 0.9, 0.4)` 이 `(0.83, 0.59, 0.33)` 이 된다 — 4.5:1 이던 색
   비율이 2.5:1 로 뭉개진다. **포화 연산 자체가 채도를 씻는다.**
   휘도만 포화시키고 그 비율을 세 채널에 똑같이 곱할 것.
2. **순수 가산이면 안 된다.** 포일은 인쇄 *위에 얹힌 층*이라 밑의 밝은 인쇄가
   그대로 비치면 색이 다 씻긴다. 띠가 센 자리에서는 포일 색으로 갈아타고
   (`_BorderCover`), 그 위에 광택만 조금 더한다.
